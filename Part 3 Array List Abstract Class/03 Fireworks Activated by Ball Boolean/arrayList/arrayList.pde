...
void draw() {
  ...
  /*if ( shapes.get(3).s==true) {
    Fireworks fireworks = new Fireworks (shapes.get(3).bx, shapes.get(3).by, shapes.get(3).w, shapes.get(3).w, shapes.get(3).c);
    shapes.add(fireworks); //Element 4, Fireworks Celebration
  } //End Create Fireworks
  */
  //
  for ( Shape s : shapes ) {
    s.draw();
